...
#define RETOUCH_INTENCE 0.7
#define TEETH_WHITENING
#define teethWhiteningCoeff 0.2
#define EYES_WHITENING
#define eyesWhiteningCoeff 0.3
#define NORMAL_LAYER
#define SOFT_SKIN
#define skinSoftIntensity 0.4
...
void main()
{
    ...
    if( maskColor.g > 1./255. )
    {
        ...
        res = whitening(res, teeth_factor * teethWhiteningCoeff, BNB_PASS_SAMPLER_ARGUMENT(lookupTexTeeth));
#endif
    }
#endif
...
#ifdef NORMAL_LAYER
    vec4 makeup2 = RETOUCH_INTENCE * BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_normalMakeup), var_uv_bg_uv.xy );
