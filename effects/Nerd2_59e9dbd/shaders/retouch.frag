...
vec3 softSkin(vec3 originalColor, float factor) {
    ...
    vec3 nextColor0 = BNB_TEXTURE_2D_LOD(BNB_SAMPLER_2D(glfx_BACKGROUND), vec2(var_uv_bg_uv.z-dx, var_uv_bg_uv.w-dy), 0.).xyz;
    vec3 nextColor1 = BNB_TEXTURE_2D_LOD(BNB_SAMPLER_2D(glfx_BACKGROUND), vec2(var_uv_bg_uv.z+dx, var_uv_bg_uv.w-dy), 0.).xyz;
    vec3 nextColor2 = BNB_TEXTURE_2D_LOD(BNB_SAMPLER_2D(glfx_BACKGROUND), vec2(var_uv_bg_uv.z-dx, var_uv_bg_uv.w+dy), 0.).xyz;
    vec3 nextColor3 = BNB_TEXTURE_2D_LOD(BNB_SAMPLER_2D(glfx_BACKGROUND), vec2(var_uv_bg_uv.z+dx, var_uv_bg_uv.w+dy), 0.).xyz;
