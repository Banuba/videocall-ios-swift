...
BNB_DECLARE_SAMPLER_LUT(4, 5, lookupTexEyes);

BNB_DECLARE_SAMPLER_LUT(2, 3, lookupTexTeeth);


BNB_DECLARE_SAMPLER_2D(0, 1, maskTex);
...
void main()
{
   ...
   vec2 uvh = vec2(abs(2.0 * (var_uv_bg_uv.x - 0.5)),var_uv_bg_uv.y);
    ...
    res.xyz = mix( res.xyz, makeup2.xyz, makeup2.w*make_alpha );
#endif

    bnb_FragColor = vec4(res,BNB_TEXTURE_2D(BNB_SAMPLER_2D(maskTex), var_uv_bg_uv.xy).x);
