...
#define SOFT_LIGHT_LAYER_HALbnb_FragColor
#define NORMAL_LAYER_HALbnb_FragColor
...
BNB_DECLARE_SAMPLER_2D(12, 13, glfx_BACKGROUND);
...
void main()
{
    ...
    #ifdef EYES_HIGHLIGHT_HALbnb_FragColor
        uv_high.x = abs(2.0 * (uv_high.x - 0.5)); 
    #endif

    res = res + vec4( BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_highlight), uv_high ).xyz, 0. );
#endif

#ifdef SOFT_LIGHT_LAYER
    vec2 uv_soft = var_uv; 

    #ifdef SOFT_LIGHT_LAYER_HALbnb_FragColor
        uv_soft.x = abs(2.0 * (uv_soft.x - 0.5)); 
    #endif

    res.xyz = blendSoftLight( res.xyz, BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_softLight), uv_soft ).xyz );
#endif

#ifdef NORMAL_LAYER
    vec2 uv_norm = var_uv; 

    #ifdef NORMAL_LAYER_HALbnb_FragColor
        uv_norm.x = abs(2.0 * (uv_norm.x - 0.5)); 
    #endif

    vec4 makeup2 = BNB_TEXTURE_2D(BNB_SAMPLER_2D(tex_normalMakeup), uv_norm );
