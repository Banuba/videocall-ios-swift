...
BNB_DECLARE_SAMPLER_2D(18, 19, bnb_STATICPOS);
#endif
#endif
...
BNB_DECLARE_SAMPLER_2D(20, 21, bnb_MORPH);
...
#ifdef GLFX_LIGHTING
    var_n = quat_rotate( bnb_QUAT, mat3(bnb_MV)*(bnb_decode_int1010102(attrib_n).xyz*mat3(m)) );
#ifdef GLFX_TBN
    var_t = quat_rotate( bnb_QUAT, mat3(bnb_MV)*(bnb_decode_int1010102(attrib_t).xyz*mat3(m)) );
    var_b = quat_rotate( bnb_QUAT, bnb_decode_int1010102(attrib_t).w*cross( var_n, var_t ) );
#endif
    var_v = quat_rotate( bnb_QUAT, (bnb_MV*vec4(vpos,1.)).xyz );
#endif
